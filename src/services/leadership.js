...
export async function getProfileLeadershipByLeader(profileId) {
...
export async function upsertProfileLeadership(profileId, payload) {
    ...
    if (payload.field_of_study)
        extra.field_of_study = payload.field_of_study.trim();
    if (payload.influence_level)
        extra.influence_level = payload.influence_level;
    // Campos específicos para Patriarca/Matriarca
    if (payload.family_size !== undefined)
        extra.family_size = payload.family_size;
    if (payload.generation_scope)
        extra.generation_scope = payload.generation_scope;
    if (payload.influence_roles && payload.influence_roles.length > 0)
        extra.influence_roles = payload.influence_roles;
    if (payload.tradition)
        extra.tradition = payload.tradition;
    if (payload.tradition_other)
        extra.tradition_other = payload.tradition_other.trim();
    // Campos específicos para Mentor/Coach/Conselheiro
    if (payload.mentorship_type)
        extra.mentorship_type = payload.mentorship_type;
    if (payload.mentorship_type_other)
        extra.mentorship_type_other = payload.mentorship_type_other.trim();
    if (payload.target_audience && payload.target_audience.length > 0)
        extra.target_audience = payload.target_audience;
    if (payload.target_audience_other)
        extra.target_audience_other = payload.target_audience_other.trim();
    if (payload.mentees_count !== undefined)
        extra.mentees_count = payload.mentees_count;
    if (payload.certification)
        extra.certification = payload.certification.trim();
    // Campos específicos para Celebridade pública
    if (payload.celebrity_area)
        extra.celebrity_area = payload.celebrity_area.trim();
    if (payload.public_role)
        extra.public_role = payload.public_role.trim();
    if (payload.audience)
        extra.audience = payload.audience.trim();
    // Campos específicos para Jornalista/Comunicador
    if (payload.media_area)
        extra.media_area = payload.media_area;
    if (payload.audience_scope)
        extra.audience_scope = payload.audience_scope;
    // Campos específicos para Influenciador digital
    if (payload.platform)
        extra.platform = payload.platform.trim();
    if (payload.niche)
        extra.niche = payload.niche.trim();
    if (payload.reach_estimate)
        extra.reach_estimate = payload.reach_estimate;
    if (payload.influence_type)
        extra.influence_type = payload.influence_type;
    // Campos específicos para Presidente de entidade
    if (payload.entity_type)
        extra.entity_type = payload.entity_type;
    if (payload.scope)
        extra.scope = payload.scope;
    if (payload.mandate_start)
        extra.mandate_start = payload.mandate_start;
    if (payload.mandate_end)
        extra.mandate_end = payload.mandate_end;
    // Campos específicos para Liderança partidária
    if (payload.party)
        extra.party = payload.party.trim();
    if (payload.party_office)
        extra.office = payload.party_office.trim();
    if (payload.party_office_other)
        extra.office_other = payload.party_office_other.trim();
    if (payload.party_scope)
        extra.scope = payload.party_scope;
    if (payload.party_status)
        extra.status = payload.party_status;
    if (payload.party_mandate_start)
        extra.mandate_start = payload.party_mandate_start;
    if (payload.party_mandate_end)
        extra.mandate_end = payload.party_mandate_end;
    // Campos específicos para Influenciador cultural
    if (payload.cultural_segment)
        extra.segment = payload.cultural_segment.trim();
    if (payload.cultural_segment_other)
        extra.segment_other = payload.cultural_segment_other.trim();
    if (payload.cultural_role)
        extra.role = payload.cultural_role.trim();
    if (payload.cultural_role_other)
        extra.role_other = payload.cultural_role_other.trim();
    if (payload.cultural_scope)
        extra.scope = payload.cultural_scope;
    if (payload.cultural_notes)
        extra.notes = payload.cultural_notes.trim();
    // Campos específicos para Educador/Professor
    if (payload.education_level)
        extra.education_level = payload.education_level;
    if (payload.subject_area)
        extra.subject_area = payload.subject_area.trim();
    if (payload.reach_scope)
        extra.reach_scope = payload.reach_scope;
    if (payload.notes)
        extra.notes = payload.notes.trim();
    ...
    if (payload.projects)
        extra.projects = payload.projects.trim();
    // Campos específicos para Militar/Forças de segurança
    if (payload.service_branch)
        extra.service_branch = payload.service_branch;
    if (payload.unit)
        extra.unit = payload.unit.trim();
    if (payload.org_custom)
        extra.org_custom = payload.org_custom.trim();
    if (payload.rank_custom)
        extra.rank_custom = payload.rank_custom.trim();
    const toInsert = {
        ...base,
...
export async function deleteProfileLeadership(profileLeadershipId) {
    if (!getSupabaseClient())
        throw new Error('Supabase client not initialized');
    // Validar UUID
