...
function getOrigin(req, body) {
  ...
  return body.appUrl || headerOrigin || refererOrigin || "http://localhost:5173";
}
async function requireAdmin(req) {
  const jwt = req.headers.get("Authorization")?.replace("Bearer ", "");
  if (!jwt) throw new Response("Unauthorized", {
    status: 401
  });
      ...
      case "list_pending":
        {
          ...
          const { data, error } = await admin.from("invite_tokens").select("id, email, full_name, role, created_at, expires_at, accepted_at").is("accepted_at", null).order("created_at", {
            ascending: false
          });
